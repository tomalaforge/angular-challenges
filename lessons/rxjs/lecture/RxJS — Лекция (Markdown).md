# RxJS — Реактивное программирование для Angular-разработчиков

> Ментальная модель потоков данных для решения реальных UI-задач
...
| Цвет | Значение |
|------|----------|
| cyan `#22D3EE` | Observable / поток данных |
| violet `#A78BFA` | оператор / преобразование |
| green `#34D399` | событие пользователя |
| coral `#FB7185` | ошибка / опасное место |
| amber `#FBBF24` | успех / результат |

---

# Раздел 1. Проблема и интуиция

Почему привычных инструментов недостаточно и что такое поток
...
> Запомни: RxJS — это Lodash для событий: те же `map` и `filter`, но для значений во времени.
...
> Запомни: В Angular почти всё асинхронное — это Observable.
...
Angular: Live search: не отправлять запрос на каждый символ, ждать окончания ввода.
...
## Что такое higher-order mapping

Когда значение потока — тоже Observable. Оператор решает, как управлять внутренними потоками.
...
Каждое событие запускает внутренний Observable (например, HTTP-запрос). Как управлять несколькими?

> Запомни: Все четыре оператора — это `map` + стратегия управления внутренними Observable.
...
| Идеально для | Live search, отмена устаревших HTTP-запросов |
...
| Опасно для | Поиск — race condition: ответы могут прийти вразнобой |
...
| Идеально для | Submit формы — защита от двойной отправки |
...
| Современный (Angular 16+) | `takeUntilDestroyed()` — без boilerplate |
...
## CHALLENGE: Live search без лишних запросов
...
Типичная ошибка: Отправлять запрос на каждый keystroke — сервер заваливается запросами, UI лагает.
...
## CHALLENGE: Сохранение формы без двойного submit
...
Типичная ошибка: Подписываться на каждый источник отдельно — race condition, данные рассинхронизируются.
...
Типичная ошибка: Не обрабатывать error — поток умирает, UI застревает в loading. `catchError` без fallback — тоже умирает.
...
Типичная ошибка: Сохранять `Subscription` в свойстве и отписываться в `ngOnDestroy` — работает, но boilerplate. Легко забыть.

---

# Раздел 9. Итоги

Cheat sheet и алгоритм выбора оператора

---

## Cheat sheet: ситуация → оператор
...
| Защита от двойного submit | `exhaustMap` | Пока занят — игнорируй клики |
...
| Перехват ошибки | `catchError` | Вернуть fallback, не уронить UI |
| Повтор запроса | `retry` | Автоматический повтор при ошибке |
| Очистка подписки | `takeUntilDestroyed` | Отписка при уничтожении |
| Побочный эффект | `tap` | Лог, отладка — без изменения потока |
| Кэширование результата | `shareReplay` | Один запрос, результат для всех |

> Запомни: Сохраните этот cheat sheet — он покрывает 90% задач из Angular Challenges.
...
→ **switchMap**
Live search, навигация, отмена устаревших запросов
...
→ **exhaustMap**
Submit формы, предотвращение двойной отправки
